	...
	void checkForApptCollision(Appointment appointment) {
		...
		curAppts.forEach(appt -> {
			...
			if (appt.getStartTime()/60 == appointment.getStartTime()/60){
				System.out.println("The requested dentist is already booked for an appointment at this requested start-time.  \nPlease select another start-time.");
				throw new AppointmentNotFoundException("The requested dentist is already booked for an appointment at this requested start-time.  \nPlease select another start-time.");
			}
		});
	}
